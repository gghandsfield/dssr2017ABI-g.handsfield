function [idx,D]=YiCao_knnsearch(varargin)
% NOTE: Renamed YiCao_knnsearch from knnsearch to not interfere with
% MATLAB's knnsearch. Changed by G Handsfield, 2016.
%
% EDITED BY GH FOR SYNTAX CONSISTENCY WITH KNNSEARCH. Namely, changed
% inputs (Q,R,K) to (R,Q,K), the latter is MATLAB convention. 
% 
% CAUTION: convevntion has been changed in comments but not in examples 
%
...
%
% IDX = knnsearch(R,Q) takes the default value K=1.
%
% IDX = knnsearch(R) or IDX = knnsearch(R,[],K) does the search for Q = R.
%
% Rationality
...
% EDITED BY GH TO BE CONSISTENT WITH KNNSEARCH
...
% Check inputs
% [Q,R,K,fident] = parseinputs(varargin{:}); %original code
[R,Q,K,fident] = parseinputs(varargin{:}); %GH altered code;
...
narginchk(1,3); %edited by gh for later than Matlab 2016 use
